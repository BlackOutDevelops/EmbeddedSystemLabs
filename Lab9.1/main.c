...
int main(void)
{
    // Manufacturer ID(0h7E) reads 0h5449
    // Device ID(0h7F) reads 0h3001
	...
	char manIDString[] = " Manufacturer ID: ",
	              devIDString[] = "  Device ID: ";
	...
	for(;;)
    {
	    ...
	    uart_write_string(manIDString);
	    uart_write_intToHex(manID);
	    uart_write_char('\n');
	    uart_write_char('\r');
	    uart_write_string(devIDString);
	    uart_write_intToHex(devID);
        uart_write_char('\n');
        uart_write_char('\r');
    }
}
